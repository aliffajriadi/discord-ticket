  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addStringOption((opt) =>
        opt
          .setName("label")
          .setDescription("Nama alasan (tampil di dropdown)")
          .setRequired(true),
      )
      .addStringOption((opt) =>
        opt
          .setName("value")
          .setDescription("ID unik alasan (huruf_kecil, tanpa spasi)")
          .setRequired(true),
      )
      ...
      .addRoleOption((opt) =>
        opt
          ...
          .setDescription("Role petugas yang menangani tiket ini")
          .setRequired(true),
      )
      .addStringOption((opt) =>
        opt
          .setName("description")
          .setDescription("Deskripsi singkat alasan (opsional)")
          .setRequired(false),
      ),
  )
  .addSubcommand((sub) =>
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addStringOption((opt) =>
        opt
          .setName("value")
          .setDescription("ID unik alasan yang akan dihapus")
          .setRequired(true),
      ),
  );
...
export async function execute(interaction: ChatInputCommandInteraction) {
  if (!interaction.guild) {
    return interaction.reply({
      embeds: [errorEmbed("Command ini hanya bisa digunakan di server.")],
      ephemeral: true,
    });
  }

  await ensureGuild(interaction.guild.id);
  const hasPermission = await hasSetupPermission(
    interaction.guild,
    interaction.user.id,
  );
  ...
  if (!hasPermission) {
    return interaction.reply({
      embeds: [
        errorEmbed("Kamu tidak memiliki izin untuk menjalankan command ini."),
      ],
      ephemeral: true,
    });
  }
  ...
  if (subcommand === "add") {
    ...
    if (!/^[a-z0-9_]+$/.test(value)) {
      return interaction.reply({
        embeds: [
          errorEmbed(
            "Value hanya boleh mengandung huruf kecil, angka, dan underscore.",
          ),
        ],
        ephemeral: true,
      });
  ...
  if (subcommand === "list") {
    ...
    if (reasons.length === 0) {
      return interaction.reply({
        embeds: [
          errorEmbed(
            "Belum ada alasan tiket. Gunakan `/reason add` untuk menambahkan.",
          ),
        ],
        ephemeral: true,
      });
  ...
  if (subcommand === "delete") {
    ...
    if (!reason) {
      return interaction.reply({
        embeds: [
          errorEmbed(`Alasan dengan value \`${value}\` tidak ditemukan.`),
        ],
        ephemeral: true,
      });
    }
    ...
    return interaction.reply({
      embeds: [
        successEmbed(
          `Alasan **${reason.label}** (\`${value}\`) berhasil dihapus.`,
        ),
      ],
      ephemeral: true,
    });
